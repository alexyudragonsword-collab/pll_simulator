...
| floor | the same library suite on **3.10** with the declared minimum `numpy`/`scipy`/`matplotlib` pinned — the interpreter and wheels the phone actually runs (`test-minimum` job; read its *selected* count, not just the colour) |
