...
class ModulationPage(Page):
    ...
    def _go(self):
        ...
        def fn():
            ...
            bits = prbs(max(64, int((n_cyc - settle) * rb / fref) - 20),
                        seed=7)
            ...
            sim = pll.simulate(n_cyc, seed=2, mod_freq=mod,
                               mod_dp_gain=1.0 + dp)
            e = evm(sim.phase_err_out[settle + 4000:],
                    ideal[settle + 4000:])
            return e, sim, mod, ideal, settle

        def done(res):
            e, sim, mod, ideal, settle = res
...
class HopSettlingPage(Page):
    ...
    def _go(self):
        ...
        hop = float(self.hop.text())
        n = int(float(self.n_cyc.text()))
        seed = int(float(self.seed.text()))

        def fn():
            pll = presets.ALL_PRESETS[nm]()
            return hop_settling(pll, pll.cfg.fout + hop, n_cycles=n,
                                seed=seed)
        ...
        def done(r):
            ...
            ax.grid(alpha=0.3)
            self.figs.set_figs([fig])
        self.run_async(fn, done, self.btn, self.btn_stats)

    def _go_stats(self):
        nm = self.preset.currentText()
        hop = float(self.hop.text())
        n = int(float(self.n_cyc.text()))
        ns = int(float(self.n_seeds.text()))

        def fn():
            return hop_statistics(
                lambda: presets.ALL_PRESETS[nm](),
                presets.ALL_PRESETS[nm]().cfg.fout + hop,
                seeds=range(ns), n_cycles=n)
